...
// laser focusing parameters
  ...
  double f_number   = 6;                      // f number of beam
  ...
  double ey_xloc = Lx * 100.0/120.0;              // x location for Ey(z) write in fft diagnostic 
...
#define DUMP_VELOCITY_DATA(SUFF,LEN,HEADER_SIZE)                    \
  ...
  if ( 1 ) {
    float        f_e[NVX*NVZ], f_He[NVX*NVZ]; 
    float        vmax_e  = 10*global->vthe,    dvx_e,  dvz_e;     
    float        vmax_He = 10*global->vthi_He, dvx_He, dvz_He; 
    FileIO       fileIO_e, fileIO_He; 
    FileIOStatus status; 
    static char  fname_e[BUFLEN], fname_He[BUFLEN]; 
